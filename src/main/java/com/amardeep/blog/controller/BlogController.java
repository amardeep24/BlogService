...
public class BlogController {
	...
	public ResponseEntity<List<BlogApi>> getAllBlogs(){
		...
		List<BlogApi> blogList=blogService.getAllBlogs()
								 .stream()
	...
	public ResponseEntity<BlogApi> createBlog(@RequestBody BlogApi blogApi,UriComponentsBuilder builder){
		...
		Author author=authorService.getAuthor(blogApi.getBlogAuthor());
		Blog blog=blogApi.getEntity();
		blog.setBlogAuthor(author);
		BlogApi createdBlog=blogService.createBlog(blog).getApi();
